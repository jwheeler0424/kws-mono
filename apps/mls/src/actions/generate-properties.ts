...
export async function generateProperties(
  ...
  standardStatus: StandardStatus | null = null,
  firstFetch = false,
  ...
  const lastModified =
    !firstFetch && !lastModifiedTimestamp
      ? await db.query.property.findFirst({
          ...
          orderBy: (property, { desc }) => desc(property.modificationTimestamp),
        })
      : null;
...
export async function generateActiveProperties(
  includeOffice = false,
  maxDate: Date | number | null = null,
  standardStatus: StandardStatus | null = StandardStatus.Active,
  firstFetch = false,
  lastModifiedTimestamp: Date | null = null
) {
  // 1. Find the most recent modification time (or default to Jan 1 of three years ago).
  ...
  do {
    // 3. Fetch a batch of properties.
    console.log(
      `Fetching active property listings (Iteration: ${iteration++})...`
    );
    const { properties, nextLink: newNext } = await throttledFetchProperties(
      minDate,
      nextLink,
...
export async function generateOfficeProperties(
  includeOffice = true,
  maxDate: Date | number | null = null,
  standardStatus: StandardStatus | null = null,
  firstFetch = false,
  ...
  do {
    // 3. Fetch a batch of properties.
    console.log(
      `Fetching office property listings (Iteration: ${iteration++})...`
    );
    const { properties, nextLink: newNext } = await throttledFetchProperties(
      minDate,
      nextLink,
