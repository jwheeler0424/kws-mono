...
export async function main() {
  try {
    ...
    logger.info('MLS full sync summary', {
      osn: summary.osn,
      mode: summary.mode,
    ...
    for (const r of summary.results) {
      logger.info('MLS full sync resource result', {
        resource: r.resource,
        upserted: r.upserted,
    ...
    if (quotaErrors.length > 0) {
      logger.warn('MLS API quota exhausted during full sync', {
    ...
    logger.fatal('MLS full sync action crashed', {
