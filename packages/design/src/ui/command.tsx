...
import { InputGroup, InputGroupAddon, InputGroupInputDebounced } from './input-group';
...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent
        className={cn('top-1/3 -translate-y-1/3 overflow-hidden rounded-xl! p-0', className)}
...
function CommandList({ className, ...props }: React.ComponentProps<typeof CommandPrimitive.List>) {
  return (
    <CommandPrimitive.List
      ...
      className={cn('no-scrollbar max-h-74 scroll-py-1 overflow-x-hidden overflow-y-auto outline-none', className)}
      {...props}
    />
  );
}
