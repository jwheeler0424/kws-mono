...
import type { NWM_Property } from '@/types/property';
...
export const properties = pgTable(
  'properties',
  {
    ...
    searchVector: tsvector('search_vector')
      ...
      .weight('A')
      // B — location (city, area codes, zip, county, subdivision)
      .cols([
        'city',
        'state_or_province',
        'postal_code',
  ...
  (t) => [
    index('idx_properties_property_type').on(t.propertyType),
    // Geo bounding box (kept for raw lat/lng queries)
    index('idx_properties_geo').on(t.latitude, t.longitude),
    ...
    index('idx_properties_h3_r6').on(t.h3R6),
    index('idx_properties_h3_r7').on(t.h3R7),
    index('idx_properties_h3_r8').on(t.h3R8),
    ...
    index('idx_properties_visible_h3_r7_status')
      ...
      .where(sql`${t.mlgCanView} = true AND ${t.deletedAt} IS NULL`),
    // Active-only status filtering path for status-only list queries.
    index('idx_properties_visible_status')
      .on(t.standardStatus, t.listingKey)
      .where(sql`${t.mlgCanView} = true AND ${t.deletedAt} IS NULL`),
    ...
    index('idx_properties_visible_featured_status')
      ...
      .where(sql`${t.mlgCanView} = true AND ${t.deletedAt} IS NULL`),
    // Composite: status + r7 — the hottest search path (featured listings near me)
    index('idx_properties_h3_r7_status_featured').on(t.h3R7, t.standardStatus, t.featuredListingYN),
    // Sync cursor
    ...
    index('idx_properties_visible_status_sort').on(
      t.mlgCanView,
      t.standardStatus,
      t.onMarketDate,
    ...
    index('idx_properties_visible_office_status_sort').on(
      t.mlgCanView,
      t.listOfficeMlsId,
      t.standardStatus,
      t.onMarketDate,
    ...
    index('idx_properties_visible_member_status_sort').on(
      t.mlgCanView,
      t.listAgentMlsId,
      t.standardStatus,
      t.onMarketDate,
    ...
    // Price sort/filter path for card queries.
    index('idx_properties_visible_price_sort').on(
      t.mlgCanView,
      t.standardStatus,
      t.listPrice,
      t.listingKey,
    ),
    // Living area range filtering path.
    index('idx_properties_visible_living_area_sort').on(
      t.mlgCanView,
      t.standardStatus,
      t.livingArea,
      t.listingKey,
    ),
    index('idx_properties_search_vector').using('gin', t.searchVector),
