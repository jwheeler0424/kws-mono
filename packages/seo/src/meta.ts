 ...
 */
export function descriptionMeta(description?: string, opts?: { maxLength?: number }): SeoMetaTag | null {
  if (!description) return null
  const maxLength = opts?.maxLength ?? 160
  const content =
    description.length > maxLength ? `${description.slice(0, maxLength - 1).trimEnd()}…` : description
