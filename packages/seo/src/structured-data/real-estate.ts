...
export function amenityFeature(name: string, value: boolean | string = true): Record<string, unknown> {
...
export type ResidenceType = 'House' | 'SingleFamilyResidence' | 'Apartment' | 'Room' | 'Suite' | 'Residence'
...
export function residenceSchema(input: ResidenceSchemaInput): JsonLdBase<ResidenceType> {
  ...
  return jsonLd({
    ...
    ...(input.numberOfBathroomsTotal !== undefined ? { numberOfBathroomsTotal: input.numberOfBathroomsTotal } : {}),
    ...(input.numberOfFullBathrooms !== undefined ? { numberOfFullBathrooms: input.numberOfFullBathrooms } : {}),
    ...
    ...(input.aggregateRating ? { aggregateRating: buildAggregateRatingValue(input.aggregateRating) } : {}),
  })
}
...
export function apartmentComplexSchema(input: ApartmentComplexSchemaInput): JsonLdBase<'ApartmentComplex'> {
  return jsonLd({
    ...
    ...(input.numberOfAccommodationUnits !== undefined
      ? { numberOfAccommodationUnits: { '@type': 'QuantitativeValue', value: input.numberOfAccommodationUnits } }
      : {}),
      ...
      ? {
        numberOfBedrooms:
          typeof input.numberOfBedrooms === 'number'
            ...
            : { '@type': 'QuantitativeValue', minValue: input.numberOfBedrooms.min, maxValue: input.numberOfBedrooms.max },
      }
      : {}),
    ...(input.petsAllowed !== undefined ? { petsAllowed: input.petsAllowed } : {}),
    ...(input.amenities?.length ? { amenityFeature: input.amenities.map((name) => amenityFeature(name)) } : {}),
    ...(input.additionalProperty?.length ? { additionalProperty: input.additionalProperty } : {}),
    ...(input.tourBookingPage ? { tourBookingPage: input.tourBookingPage } : {}),
    ...(input.review ? { review: buildReviewValue(input.review) } : {}),
    ...(input.aggregateRating ? { aggregateRating: buildAggregateRatingValue(input.aggregateRating) } : {}),
...
export interface RealEstateListingSchemaInput {
  ...
  review?: ReviewSchemaInput | JsonLdBase<'Review'> | Array<ReviewSchemaInput | JsonLdBase<'Review'>>
  aggregateRating?: AggregateRatingSchemaInput | JsonLdBase<'AggregateRating'>
  openHouse?: OpenHouseSchemaInput | JsonLdBase<'Event'> | Array<OpenHouseSchemaInput | JsonLdBase<'Event'>>
}
...
export function realEstateListingSchema(input: RealEstateListingSchemaInput): JsonLdBase<'RealEstateListing'> {
  return jsonLd({
    ...
    offers: {
      ...
      ...(input.offers.businessFunction ? { businessFunction: `https://schema.org/${input.offers.businessFunction}` } : {}),
...
export function realEstateAgentSchema(input: RealEstateAgentSchemaInput): JsonLdBase<'RealEstateAgent'> {
  return jsonLd({
    '@type': 'RealEstateAgent',
    name: input.name,
    ...(input.image ? { image: input.image } : {}),
...
export function propertyDetailPageSchema(input: PropertyDetailPageSchemaInput): JsonLdBase<'ItemPage'> {
  return jsonLd({
    '@type': 'ItemPage',
    url: input.url,
    name: input.name,
